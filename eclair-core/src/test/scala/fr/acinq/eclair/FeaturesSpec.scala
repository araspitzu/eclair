...
class FeaturesSpec extends FunSuite {
  ...
  test("features compatibility") {
    ...
    assert(areSupported(Protocol.writeUInt64(1L << CHANNEL_WUMBO_OPTIONAL, ByteOrder.BIG_ENDIAN)))
    assert(areSupported("14") == false)
    assert(areSupported("0141") == false)
  }

  test("'wumbo_local' feature") {
    assert(hasFeature("0100", Features.CHANNEL_WUMBO_MANDATORY))
    assert(hasFeature("0200", Features.CHANNEL_WUMBO_OPTIONAL))
  }
