...
class IntegrationSpec extends TestKit(ActorSystem("test")) with BitcoindService with FunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("open a wumbo channel and wait for longer than the default min_depth") {
    ...
    sender.send(fundee.switchboard, 'peers)
    val Some(fundeeFunderPeer) = sender.expectMsgType[Iterable[ActorRef]].find { peerRef =>
      sender.send(peerRef, GetPeerInfo)
      sender.expectMsgType[PeerInfo].nodeId == funder.nodeParams.nodeId
    }

    // wait until disconnected
    awaitCond({
      sender.send(fundeeFunderPeer, GetPeerInfo)
      sender.expectMsgType[PeerInfo].state == "DISCONNECTED"
    })
    ...
    // check the fundee is waiting for more conf, funder is waiting for fundee to send funding_locked
    awaitCond({
      sender.send(fundee.register, Forward(channelId, CMD_GETSTATE))
