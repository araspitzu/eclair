...
object Helpers {
  ...
  def validateParamsFundee(nodeParams: NodeParams, open: OpenChannel): Unit = {
    ...
    if (open.fundingSatoshis < nodeParams.minFundingSatoshis || (open.fundingSatoshis >= Channel.MAX_FUNDING_SATOSHIS  && !nodeParams.wumboEnabled) ) throw InvalidFundingAmount(open.temporaryChannelId, open.fundingSatoshis, nodeParams.minFundingSatoshis, Channel.MAX_FUNDING_SATOSHIS)
